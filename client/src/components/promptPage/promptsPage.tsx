    ...
    <div className="enter-workspace flex-1 overflow-y-auto px-7 py-7">
      <div className="max-w-[1240px] mx-auto">
        <div className="flex items-end justify-between mb-7">
          <div>
            <div className="workspace-label mb-2">Prompt system</div>
            ...
            <div className="bg-background rounded-lg border shadow-md p-6 w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                ...
                <div>
                    ...
                    {CATEGORY_OPTIONS.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center gap-1.5 rounded-lg border p-3 transition-all text-sm ${
                            isActive
                  ...
                  {editing && (
                    <p className="text-xs text-muted-foreground mt-1.5">编辑时不可修改类别，如需更换请新建模板</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="h-9 mt-1" placeholder="输入模板标题" />
                </div>

                {/* 描述 */}
                <div>
                  <Label className="text-sm text-muted-foreground">描述</Label>
                  <Input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="h-9 mt-1" placeholder="简短描述模板用途" />
                </div>
            ...
            {categories.map((cat) => {
              ...
              return (
                <TabsContent key={cat} value={cat} className="mt-0 p-5">
                  {filtered.length === 0 ? (
                    <div className="text-center py-12 text-muted-foreground text-sm">该分类暂无模板</div>
                  ) : (
