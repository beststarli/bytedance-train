...
router.get('/works', async (req, res) => {
    ...
    if (!userId)
        return;
    const { rows } = await db_1.pool.query('SELECT id, title, content, status, quality_score, view_count, created_at, updated_at FROM works WHERE user_id = $1 ORDER BY updated_at DESC', [userId]);
    res.json({ works: rows });
});
